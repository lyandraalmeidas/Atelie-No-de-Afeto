...
import { CreateCategoryInput, UpdateCategoryInput, PaginationQuery } from '../types';
...
export class CategoryService {
  ...
  async create(input: CreateCategoryInput): Promise<CategoryBase> {
    if (!input.name?.trim()) throw new AppError('Nome da categoria é obrigatório.', 400);
  ...
  private async ensureNameAvailableForUpdate(name: string, id: string): Promise<void> {
    const existing = await this.categoryRepository.findByName(name);
    if (existing && existing.id !== id) throw new AppError('Já existe uma categoria com esse nome.', 409);
  }
}