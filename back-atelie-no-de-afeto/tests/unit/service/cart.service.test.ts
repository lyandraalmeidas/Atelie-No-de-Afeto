...
const MockProductRepo = ProductRepository as jest.MockedClass<
...
describe("CartService", () => {
  ...
  describe("addItem", () => {
    it("adiciona item quando produto tem estoque", async () => {
      ...
      cartRepo.upsertItem.mockResolvedValue(
        makeCart([{ productId: "prod-1", quantity: 2 }]),
      );
    ...
    it("lança AppError 400 para quantidade zero", async () => {
      await expect(
        service.addItem("user-1", "prod-1", 0),
    ...
    it("lança AppError 400 quando produto está esgotado", async () => {
      ...
      await expect(
        ...
        statusCode: 400,
        message: expect.stringContaining("esgotado"),
      });
    });
    ...
    it("lança AppError 400 quando quantidade excede estoque disponível", async () => {
      ...
      cartRepo.findOrCreate.mockResolvedValue(makeCart());
      await expect(
        service.addItem("user-1", "prod-1", 5),
      ).rejects.toMatchObject({
        statusCode: 400,
    ...
    it("lança AppError 400 quando soma com itens existentes excede estoque", async () => {
      ...
      cartRepo.findOrCreate.mockResolvedValue(
        makeCart([{ productId: "prod-1", quantity: 4 }]),
      );
      await expect(
        service.addItem("user-1", "prod-1", 2),
      ).rejects.toMatchObject({
        statusCode: 400,
        message: expect.stringContaining("insuficiente"),
      });
    ...
    it("aceita quantidade exatamente igual ao estoque disponível", async () => {
      ...
      cartRepo.findOrCreate.mockResolvedValue(makeCart());
      cartRepo.upsertItem.mockResolvedValue(
        makeCart([{ productId: "prod-1", quantity: 5 }]),
      );
  ...
  describe("updateItem", () => {
    it("atualiza quantidade quando dentro do estoque", async () => {
      productRepo.findById.mockResolvedValue(makeProduct(10));
      cartRepo.updateItemQuantity.mockResolvedValue(
        makeCart([{ productId: "prod-1", quantity: 3 }]),
      );
    ...
    it("lança AppError 400 para quantidade negativa", async () => {
      await expect(
        service.updateItem("user-1", "prod-1", -1),
      ).rejects.toMatchObject({ statusCode: 400 });
    });
    ...
    it("lança AppError 400 quando nova quantidade excede estoque", async () => {
      ...
      await expect(
        service.updateItem("user-1", "prod-1", 5),
      ).rejects.toMatchObject({
        statusCode: 400,
        message: expect.stringContaining("insuficiente"),
      });
